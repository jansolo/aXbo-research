...
public class MetaDataTableModel extends AbstractTableModel
{
  ...
  public void filterData(String name, Date dateFrom, Date dateUntil)
  {
    boolean searchName = (name != null && name.trim().length() > 0);
    boolean searchDateFrom = (dateFrom != null);
    boolean searchDateUntil = (dateUntil != null);
    ...
    if (searchName || searchDateFrom || searchDateUntil)
    {
      ...
      {
        String recName = (record.getName() != null ? record.getName() : "");
        boolean matchesName = (searchName && recName.toLowerCase().indexOf(name.
            toLowerCase()) > -1);
        boolean matchesDateFrom = (searchDateFrom && dateFrom.before(record.
            calculateStartTime()));
        boolean matchesDateUntil = (searchDateUntil && dateUntil.after(record.
            calculateStartTime()));
        if ((matchesName || !searchName) && (matchesDateFrom || !searchDateFrom) && (matchesDateUntil
            || !searchDateUntil))
        {
